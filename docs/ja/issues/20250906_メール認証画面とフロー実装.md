...
- [ ] EmailVerifyPage用の状態管理
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/stores/emailVerifyPageStore.ts`
  - 概要:
    - ページレベルの状態管理（ローディング状態など）
    - 登録時のメールアドレス保持
    - 認証チェック関連の状態管理
  - 参考: `packages/frontend/src/features/auth/create-user-page/stores/userRegisterPageStore.ts`
  - Enum定義:
    - `VerificationStatus`: 認証状態の列挙型
  ...
  - プロパティ:
    ...
    - `verificationStatus`: 認証状態（VerificationStatus型）
...
- [x] メール認証説明テキストコンポーネント
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/components/VerificationInstructionText.tsx`
  - 概要:
...
- [x] 認証ステータス表示コンポーネント
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/components/VerificationStatusIndicator.tsx`
  - 概要:
    - 認証チェック中のローディング表示
    - 認証完了時のチェックマーク表示
    - ステータス変化のアニメーション
...
- [x] ヘルプ・トラブルシュート表示コンポーネント
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/components/EmailVerificationHelpSection.tsx`
  - 概要:
    - 「メールが届かない場合は？」等のヘルプ情報
    - 折りたたみ式表示
    - スパムフォルダ確認等のアドバイス表示
...
- [ ] EmailVerifyPage画面コンポーネント
  ...
  - 概要: 
    ...
    - AppStateの変化を監視して認証状態チェックを実行
    - 認証完了時の自動遷移処理
  - 参考: `packages/frontend/src/features/auth/create-user-page/UserRegisterPage.tsx`

### フロントエンド（フック）
- [ ] メール認証完了チェック用のカスタムフック
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/hooks/useEmailVerificationChecker.ts`
  - 概要:
    - AppState監視機能
    - Supabaseから認証状態取得
    - 認証完了時のコールバック実行
  - 参考: `packages/frontend/src/features/auth/create-user-page/hooks/useUserRegisterHandler.ts`

- [ ] 確認メール再送用のカスタムフック
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/hooks/useResendVerificationEmail.ts`
  - 概要:
    - Supabaseでメール再送処理
    - エラーハンドリング
    - ローディング状態管理
...
- [ ] 自動ログイン処理用のカスタムフック
  - ファイルパス: `packages/frontend/src/features/auth/email-verify-page/hooks/useAutoLogin.ts`
  - 概要:
    - 認証完了ユーザーの自動ログイン処理
    - セッション確立
    - ホーム画面への遷移
...
- [ ] カスタムフックのテスト
  ...
  - 概要:
    ...
    - useResendVerificationEmailのテスト
    - useAutoLoginのテスト
