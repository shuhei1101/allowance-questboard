...
-- 学歴マスタ
5. 05_insert_data/member/education.sql
...
12. 05_insert_data/member/families.sql
```

### 4. メンバーデータ（families, educationに依存）
```sql
-- メンバーテーブル（families, icons, educationに依存）
13. 05_insert_data/member/members.sql

-- メンバー学年（members, educationに依存）
14. 05_insert_data/member/member_grade.sql

-- 教育期間（members, educationに依存）
15. 05_insert_data/member/education_period.sql

-- フォロー関係（familiesに依存）
16. 05_insert_data/member/follows.sql

-- 経験値レベル設定（familiesに依存）
17. 05_insert_data/member/exp_by_level.sql
```

### 5. サブタイプテーブルデータ
```sql
-- お小遣いテーブルサブタイプ
18. 05_insert_data/member/allowance_table_sub_types.sql

-- レベルテーブルサブタイプ
19. 05_insert_data/member/level_table_sub_types.sql
...
### 6. レベルテーブル関連データ
```sql
-- レベルテーブル関連（members, families, member_level_sub_typesに依存）
22. 05_insert_data/member/level_table.sql
```

### 7. お小遣いテーブル関連データ
```sql
-- お小遣いテーブル関連（members, families, allowance_table_sub_typesに依存）
23. 05_insert_data/member/allowance_table.sql
```

### 8. クエスト関連データ
```sql
-- クエストカテゴリ（quest_category_subclass_types, familiesに依存）
24. 05_insert_data/quest/quest_category.sql

-- クエスト関連（quest_subclass_types, quest_category, icons, currenciesに依存）
25. 05_insert_data/quest/quests.sql
   ※このファイルには以下のテーブルのINSERT文が含まれます：
   - quests（基底クエストテーブル）
   - template_quests（テンプレートクエスト）
   - quest_translations（クエスト翻訳）
   - member_quest_status（メンバークエストステータス）
   - member_quest_status_translations（ステータス翻訳）
   - family_quests（家族クエスト）
   - shared_quests（共有クエスト）
   - saved_quests（保存クエスト）
   - member_quests（メンバークエスト）
   - quest_details_by_level（クエスト詳細レベル別）
   - quest_details_by_level_translations（クエスト詳細翻訳）
   - quest_request_status（クエストリクエストステータス）
   - quest_request_status_translations（リクエストステータス翻訳）
...
### サブタイプテーブル
- **allowance_table_sub_types**: お小遣いテーブルのサブタイプ（member, family, shared）
- **member_level_sub_types**: レベルテーブルのサブタイプ（member, family, shared）
- **quest_subclass_types**: クエストサブクラスタイプ（template, family, custom）
- **quest_category_subclass_types**: クエストカテゴリサブクラスタイプ（template_quest_category, family_quest_category, custom_quest_category）
...
- template_quests（テンプレートクエスト）
- quest_translations（クエスト翻訳）: 日本語・英語対応

**クエスト進行管理:**
- member_quest_status（メンバークエストステータス）: assigned, in_progress, completed等
- member_quest_status_translations（ステータス翻訳）
- member_quests（メンバークエスト）: 各メンバーのクエスト進行状況
...
**クエストリクエスト:**
- quest_request_status（クエストリクエストステータス）: pending, approved, rejected等
- quest_request_status_translations（リクエストステータス翻訳）
- quest_requests（クエストリクエスト）: メンバーからの新規/既存クエストリクエスト

**クエストカテゴリ（別ファイル: quest_category.sql）:**
...
4. **icons** → **アイコン参照データ**
5. **families** → **members** → **各機能データ**

### 特に注意が必要な依存関係
- **auth.users**: 認証システムで事前作成が必要
- **families**: auth.usersとiconsに依存
- **members**: familiesとeducationに依存
