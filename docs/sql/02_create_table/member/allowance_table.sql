...
COMMENT ON COLUMN shared_allowance_table.created_at IS 'レコードの作成日時';
COMMENT ON COLUMN shared_allowance_table.updated_at IS 'レコードの更新日時';

-- member_allowance_table(履歴)
CREATE TABLE IF NOT EXISTS history.member_allowance_table (
    id serial PRIMARY KEY,
...
COMMENT ON TABLE history.member_allowance_table IS 'メンバーお小遣い設定の変更履歴を管理するテーブル';
