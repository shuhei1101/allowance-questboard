...
-- 貯金記録翻訳テーブル
CREATE TABLE IF NOT EXISTS savings_records_translations (
    id serial PRIMARY KEY,
    ...
    UNIQUE (savings_record_id, language_id)
);

COMMENT ON TABLE savings_records_translations IS '貯金記録の多言語対応テーブル';
COMMENT ON COLUMN savings_records_translations.id IS '翻訳ID（主キー）';
COMMENT ON COLUMN savings_records_translations.savings_record_id IS '貯金記録ID（外部キー）';
COMMENT ON COLUMN savings_records_translations.language_id IS '言語ID（外部キー）';
COMMENT ON COLUMN savings_records_translations.reason IS '貯金の理由の翻訳';
