...
class DBManager:
    ...
    def create_tables(self):
        ...
        print("テーブルを作成中...")
        try:
            sorted_tables = DB_CONF.Base.metadata.sorted_tables
            for table in sorted_tables:
                table.create(bind=DB_CONF.engine, checkfirst=True)
            print("テーブルの作成が完了しました")

            db = DB_CONF.SessionLocal()
            # 作成されたテーブルを確認
            try:
                result = db.execute(
                    text(
    ...
    def force_drop_all_public_tables(self):
