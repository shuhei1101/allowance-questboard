...
class WithdrawalRequestsEntity(BaseEntity):
    ...
    requested_by = Column(Integer, ForeignKey("children.id", ondelete="CASCADE"), nullable=False, comment="申請者の子供ID")
    approved_by = Column(Integer, ForeignKey("families.id", ondelete="CASCADE"), nullable=False, comment="承認者の家族ID")
    status_id = Column(Integer, ForeignKey("withdrawal_request_statuses.id", ondelete="SET NULL"), nullable=False, comment="申請ステータスID")
    amount = Column(Integer, nullable=False, comment="引き落とし金額")
    reason = Column(String(500), nullable=False, comment="引き落とし理由")
    requested_at = Column(DateTime(timezone=True), nullable=False, comment="申請日時")
