...
export class QuestMembersEntity extends AppBaseEntity {
  ...
  family_quest!: any; // Avoid circular import
...
export class QuestMembersHistoryEntity extends BaseHistoryEntity {
  ...
  @Column({ type: "int", nullable: false, comment: "現在のレベル" })
  current_level!: number;

  @Column({ type: "int", nullable: false, comment: "クエストステータスID" })
  status_id!: number;

  @Column({ type: "timestamp", nullable: false, comment: "クエスト公開日時" })
  published_at!: Date;
  ...
  protected static setSpecificAttrs(
    instance: QuestMembersHistoryEntity, 
    source: QuestMembersEntity
  ): void {
